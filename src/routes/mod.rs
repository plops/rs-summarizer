...
mod tests {
    ...
    fn index_template_exposes_accessible_thinking_selector() {
        ...
        assert!(html.contains(&format!("rs-summarizer v{}", crate::APP_VERSION)));
    }

    #[tokio::test]
    async fn browse_shows_runtime_and_persisted_summary_versions() {
        let state = build_test_state().await;
        sqlx::query(
            "INSERT INTO summaries (model, summary, summary_done, rs_summarizer_version) VALUES (?, ?, ?, ?)",
        )
        .bind("test-model")
        .bind("completed")
        .bind(true)
        ...
        .await
        .unwrap();
        sqlx::query("INSERT INTO summaries (model, summary_done) VALUES (?, ?)")
            .bind("legacy-model")
            .bind(true)
            .execute(&state.db)
            .await
        ...
        let response = browse_summaries(
            State(state.clone()),
            ConnectInfo("127.0.0.1:8080".parse().unwrap()),
            HeaderMap::new(),
            Query(crate::models::BrowseParams { page: None }),
        )
        .await
        .into_response();
        ...
        assert!(html.contains(&format!("rs-summarizer v{}", crate::APP_VERSION)));
        assert!(html.contains("Generated by rs-summarizer v1.2.3"));
        assert!(html.contains("Generated by rs-summarizer Version unbekannt"));
    }
